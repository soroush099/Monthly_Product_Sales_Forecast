...
def plot_results_comparison(data, df_predictions_pivoted, codes_to_plot, output_dir, external_csv=None):
    ...
    for code_to_plot in codes_to_plot:
        ...
        for year, color, marker, style in [(1402, 'black', 'o', '-'),
                                           (1403, 'red', 's', '-'),
                                           (1404, 'green', 'D', '-')]:
            year_data = actual_data[actual_data['Year'] == year]
            if not year_data.empty:
                plt.plot(year_data['Month'], year_data['MainQty'],
        ...
        if code_to_plot in df_predictions_pivoted.index.get_level_values('Code'):
            ...
            plt.plot(months_numeric, prediction_data.values,
                     marker='^', linestyle='--', color='blue',
                     label='Prediction 1404')

        if external_data is not None:
            ext_points = external_data[external_data['Code'] == code_to_plot]
            if not ext_points.empty:
                plt.scatter(ext_points['Month'], ext_points['MainQty'],
